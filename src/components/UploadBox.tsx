...
import { analyzeAndSaveReport } from "@/services/report-service";
...
export function UploadBox({ onUploaded }: { onUploaded: () => Promise<void> }) {
  const { user } = useAuth();
  ...
  async function handleUpload() {
    ...
    try {
      const response = await analyzeAndSaveReport(user.uid, activeProfile.id, file);
      setResult(response.analysis);
      setWasSaved(response.saved ?? false);
      await onUploaded();
    } catch (uploadError) {
      setError(uploadError instanceof Error ? uploadError.message : "Failed to analyze report.");
    } finally {
      setIsLoading(false);
    }
  }
      ...
      <div className="grid gap-4 rounded-[1.75rem] border border-slate-200/80 bg-gradient-to-br from-white to-sky-50/70 p-4 md:grid-cols-[1fr_auto] md:items-center">
        ...
        >
          <span className="inline-flex items-center gap-2">
            {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4 text-sky-300" />}
        ...
        <div className="space-y-6 rounded-[2rem] border border-[#dbe8f4] bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,0.12),transparent_22%),linear-gradient(180deg,#f8fbff_0%,#eef6ff_100%)] p-4 text-slate-950 shadow-[0_24px_60px_rgba(15,23,42,0.08)] sm:p-6">
            ...
            <div className="grid gap-3 text-sm md:min-w-72">
              ...
              <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-1">
                ...
                <div className="rounded-2xl border border-white/80 bg-white/75 p-4 shadow-sm">
                  ...
                  <p className="mt-2 font-medium text-slate-950">{result.metadata.lab_name}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="w-fit rounded-full border border-white/80 bg-white/85 px-3 py-1 text-sm text-slate-600 shadow-sm">
                {result.analysis.abnormal_values.length} finding{result.analysis.abnormal_values.length === 1 ? "" : "s"}
              </span>
            </div>
            ...
            {result.analysis.abnormal_values.length > 0 ? (
              <div className="grid gap-3 md:grid-cols-2">
                {result.analysis.abnormal_values.map((item) => {
                  ...
                  return (
                    <article key={`${item.test}-${item.value}`} className={`rounded-[1.5rem] border p-4 shadow-sm ${tone.panel}`}>
                      ...
                      <p className="mt-3 text-sm leading-6 text-slate-700">{item.simple_explanation}</p>
                    </article>
                  );
                })}
              </div>
          ...
          {result.analysis.abnormal_values.length > 0 ? (
            <div className="rounded-[1.75rem] border border-white/80 bg-white/85 p-5 shadow-sm">
              ...
              >
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Full analysis</p>
                  <h4 className="mt-2 text-lg font-semibold text-slate-950 sm:text-xl">
                ...
                <div className="shrink-0 rounded-full border border-slate-200 bg-slate-50 p-3 text-slate-700">
                  {showFullAnalysis ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                </div>
              </button>
              ...
              {showFullAnalysis ? (
                <div className="mt-5 space-y-3 border-t border-slate-200 pt-5">
                  {result.analysis.abnormal_values.map((item) => {
                    ...
                    return (
                      <article key={`detail-${item.test}-${item.value}`} className={`rounded-[1.75rem] border p-5 shadow-[0_14px_30px_rgba(15,23,42,0.05)] ${tone.panel}`}>
